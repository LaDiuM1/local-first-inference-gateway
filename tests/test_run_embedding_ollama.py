"""임베딩 Ollama 감독 스크립트 검증 — 실제 Ollama 없이 가짜 자식으로 결정적으로 확인한다.

가짜 자식은 실행마다 자기 PID를 파일에 덧붙이고, 짧게 끝나거나(재기동 검증) 오래 살아 있는다(정리 검증).
이를 통해 최초 시작·종료 후 재기동·backoff 대기·감독 종료 시 자식 정리를 sleep 없이 관측한다. 운영
인터페이스는 그대로 두고 테스트는 `-ServeCommand`로 가짜 명령만 주입한다.

살아 있는 자식 정리는 kill-on-close 잡 오브젝트가 보장하므로, 감독을 강제 종료(finally가 실행되지 않는
경로)해도 자식이 정리되는지를 시그널 없이 결정적으로 검증한다. PowerShell 5.1은 Ctrl+Break에서 finally를
실행하지 않고 디버거로 진입하므로, OS 시그널이 아니라 잡 오브젝트가 정리 계약의 근거다.
"""
