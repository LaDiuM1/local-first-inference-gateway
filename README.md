...
![tests](https://img.shields.io/badge/tests-554_passed-success)
...
```mermaid
flowchart TB
    C["openAt MSA 클라이언트 (사용 기술 독립)"]
    T["Cloudflare Named Tunnel"]
    G["추론 게이트웨이 (FastAPI)<br/>서비스별 API 키 인증 · 별칭 라우팅 · SSE 스트리밍 중계 · 요청 관측(JSON Lines)"]
    O["Ollama<br/>chat·vision → gemma4:12b-it-qat (GPU)<br/>embed → snowflake-arctic-embed2 (CPU 전용 인스턴스)"]
    F["OpenAI gpt-5-mini<br/>chat·vision만 폴백 (embed는 폴백하지 않음)"]
    C -- "/v1/chat/completions · /v1/responses · /v1/embeddings<br/>(OpenAI 호환, model = 공개 별칭)" --> T
    T --> G
    G -- "1차 경로" --> O
    G -. "로컬 장애 시" .-> F
```
...
> [!NOTE]
> 동시 요청은 8건까지 오류와 VRAM 초과 없이 처리되지만, 로컬 추론이 직렬이라 대기 시간이 급증한다(동시 8건 첫 토큰 p95 40.4초).

공개 객관 셋 KMMLU 48문항은 로컬 31/48로 비교 모델(각 32/48)과 동급이다. 측정 조건과 재현 경로는 [벤치마크 결과](docs/BENCHMARK_RESULTS.md), 해석과 운영 과제는 [벤치마크 피드백](docs/BENCHMARK_FEEDBACK.md)에 있다.
