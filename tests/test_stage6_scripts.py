...
def test_chat_ollama_runner_sets_loopback_gpu_and_cloud_contract(
    ...
    fake_child.write_text(
        """\
import json, os, sys
names = ["OLLAMA_HOST", "OLLAMA_CONTEXT_LENGTH", "OLLAMA_KEEP_ALIVE", "OLLAMA_MAX_LOADED_MODELS", "OLLAMA_NUM_PARALLEL", "OLLAMA_NO_CLOUD", "OLLAMA_API_KEY", "CUDA_VISIBLE_DEVICES"]
...
def test_embedding_ollama_runner_sets_loopback_cpu_and_cloud_contract(
    tmp_path: Path,
) -> None:
    output = tmp_path / "environment.json"
    fake_child = tmp_path / "write_environment.py"
    fake_child.write_text(
        """\
import json, os, sys
names = ["OLLAMA_HOST", "OLLAMA_KEEP_ALIVE", "OLLAMA_MAX_LOADED_MODELS", "OLLAMA_NUM_PARALLEL", "OLLAMA_NO_CLOUD", "OLLAMA_API_KEY", "CUDA_VISIBLE_DEVICES"]
with open(sys.argv[1], "w", encoding="utf-8") as handle:
    json.dump({name: os.environ.get(name) for name in names}, handle)
""",
        encoding="utf-8",
    ...
    serve = _array([sys.executable, str(fake_child), str(output)])
    script = SCRIPTS / "run_embedding_ollama.ps1"
    command = f"& '{script}' -ServeCommand {serve} -MaxRestarts 1"
