...
기한은 요청 도착부터 계산하며 본문 수신에 쓴 시간도 포함된다. 모든 엔드포인트의 전체 기한은 115초다. 로컬 추론은 90초 안에 응답을 시작해야 하며, 그 안에 시작하지 못하면 `chat`·`vision`은 남은 기한 동안 폴백을 시도하고 `embed`는 폴백 없이 그대로 기한 초과로 끝난다. 일반 응답은 유효한 전체 본문, 스트리밍 응답은 첫 유효 SSE 이벤트를 확보한 시점을 응답 시작으로 본다. 기한을 넘기면 `504 Gateway Timeout`을 반환한다.
...
모든 응답에는 `x-request-id` 헤더가 포함된다. 서버는 요청별 처리 기록을 이 식별자로 남기므로, 오류나 지연을 문의할 때 해당 응답의 `x-request-id` 값을 함께 전달하면 서버 기록과 대조해 원인을 추적할 수 있다.
